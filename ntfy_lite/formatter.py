...
class TemplateFormatter(Formatter):
    ...
    Handles character limits by applying a template format (e.g. {head}...{tail})
    while avoiding ntfy's attachment mechanism entirely.
    """
    ...
    def process(
        ...
        if len(msg_bytes) > self.max_length:
            # Calculate static part length
            static_template_part = (
                self.template
                .replace("{head}", "")
                .replace("{tail}", "")
                .replace("{truncation_message}", self.truncation_message)
            )
            static_bytes_len = len(static_template_part.encode("utf-8"))

            available_length = self.max_length - static_bytes_len

            if available_length <= 0:
                truncated_str = static_template_part
            else:
                ...
                truncated_str = self.template.format(
                    head=head_str, tail=tail_str, truncation_message=self.truncation_message
                )
